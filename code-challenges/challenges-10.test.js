...
const createServer = () => {
  ...
  app.get('/favoritefoods', (req, res)=>{
    res.send('Tibs, Tire, Zilzil, Pasta');
  })
  app.use('*', (req, res)=>{
    res.status(404).send('sorry page unavailable');
  })
...
const count = (target, input) => {
  var integerCount = 0;
  input.map(newInteger =>{
    newInteger.map(oldArray=>{
      if(target === oldArray){
        integerCount++;
      }
    })
...
const totalSum = (input) => {
  var sumOfIntegerArray=0;
    input.forEach(newArray =>{
      newArray.forEach(oldArray =>{
        sumOfIntegerArray += oldArray;
      })
    })
    return sumOfIntegerArray;
};
